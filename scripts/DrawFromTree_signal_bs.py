...
for f in fileNames:
  ...
  tree.Project(h_allCuts.GetName(), var,'deltaETAjj < 1.3 && (( QG1stJet >='+str(qgcut)+'&& QG2ndJet <='+str(qgcut)+')||( QG1stJet <=' +str(qgcut) + '&& QG2ndJet >='+str(qgcut)+'))')
...
outFile = TFile(outputDir+"histo_signal_bkg_"+var+"_fromTree_bs_"+str(qgcut)+".root", "recreate")
outFile.cd()
...
can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts.svg')
can_allCuts.Close()
