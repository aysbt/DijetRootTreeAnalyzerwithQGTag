...
parser.add_option("--qgcut",action="store",type="float",default="1",dest="qgcut")
...
for f in fileNames:
  ...
  #standard
 # tree.Project(h_allCuts.GetName(), var,'deltaETAjj<1.3 && mjj > '+str(minX_mass)+'&&'(('QG1stJet >='+str(qgcut)+'&& QG2ndJet <='+str(qgcut))'||'('QG1stJet <='+str(qgcut)+'&& QG2ndJet >='+str(qgcut)))) 
  # tree.Project(h_allCuts.GetName(), var,'deltaETAjj<1.3 && mjj > '+str(minX_mass)+'&& QG1stJet >='+str(qgcut)+ '&& QG2ndJet <='+str(qgcut)+'|| deltaETAjj<1.3 && mjj > '+str(minX_mass)+'&& QG1stJet <='+str(qgcut)+ '&& QG2ndJet >='+str(qgcut))
  tree.Project(h_allCuts.GetName(), var,'deltaETAjj<1.3 && mjj > '+str(minX_mass)+'&& (( QG1stJet >='+str(qgcut)+'&& QG2ndJet <='+str(qgcut)+')||( QG1stJet <=' +str(qgcut) + '&& QG2ndJet >='+str(qgcut)+'))' )
...
if golden:
 chain.Project("h_dat", var,'deltaETAjj<1.3 && mjj > '+str(minX_mass)+'&&(( QG1stJet >='+str(qgcut)+'&& QG2ndJet <='+str(qgcut)+')||( QG1stJet <='+str(qgcut)+'&& QG2ndJet >='+str(qgcut)+')) && run >= '+ str(runMin) + '&& run <= ' + str(runMax) +' && PassJSON==1')
else:
 chain.Project("h_dat", var,'deltaETAjj<1.3 && mjj > '+str(minX_mass)+'&& (( QG1stJet >='+str(qgcut)+'&& QG2ndJet <='+str(qgcut)+')||( QG1stJet <='+str(qgcut)+'&& QG2ndJet >='+str(qgcut)+')) && run >= '+ str(runMin) + ' && run <= ' + str(runMax) )
...
outFile = TFile(outputDir+"histo_data_"+var+"_fromTree"+str(qgcut)+".root", "recreate")
outFile.cd()
#scale to data
...
can_allCuts.Write()   
if(logy):
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts_logy.C')
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts_logy.png')
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts_logy.pdf')
if(rebin == -1):
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts_varBin.C')
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts_varBin.png')
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts_varBin.pdf')


else:
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts.C')
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts.png')
  can_allCuts.SaveAs(outputDir+var+str(qgcut)+'_allCuts.pdf')
